...
backup_path = r"c:\Users\sayan\Downloads\biochem Note X\BIOCHEM_MODULE_4\module04_protein_haemoglobin_X.html"

# Reset file from backup
print("Resetting module04_protein_haemoglobin.html from backup...")
shutil.copy(backup_path, html_path)

with open(html_path, 'r', encoding='utf-8') as f:
    content = f.read()

# --- Phase 4: Head Cleanup and CSS Enforcement ---
print("Applying Phase 4: Head Cleanup...")
...
new_head = """  <head>
    ...
    <link rel="stylesheet" href="wbuhs_master_style.css" />
  </head>"""
...
for original, replacement in h2_replacements.items():
    if original not in content:
        print(f"ERROR: H2 header not found: {original}")
        sys.exit(1)
...
if s1_old not in content:
    print("ERROR: Section 1 end string not found!")
    sys.exit(1)
...
if s4_old not in content:
    print("ERROR: Section 4 end string not found!")
    sys.exit(1)
...
if s6_old not in content:
    print("ERROR: Section 6 end string not found!")
    sys.exit(1)
...
if s8_old not in content:
    print("ERROR: Section 8 end string not found!")
    sys.exit(1)
...
# Section 7 (Corrected Regex: Stops at </h2>\s*</div> instead of first </div>)
s7_pattern = r'(<div id="s7">\s*<div class="section-header">.*?</h2>\s*</div>)(.*?)(</div>\s*<div class="divider">)'
if not re.search(s7_pattern, content, re.DOTALL):
    print("ERROR: Section 7 boundary pattern not found!")
    sys.exit(1)
...
# Section 9 (Corrected Regex: Stops at </h2>\s*</div> instead of first </div>)
s9_pattern = r'(<div id="s9">\s*<div class="section-header">.*?</h2>\s*</div>)(.*?)(</div>\s*<div class="divider">)'
if not re.search(s9_pattern, content, re.DOTALL):
    print("ERROR: Section 9 boundary pattern not found!")
    sys.exit(1)
...
print("SUCCESS: HTML transformations applied successfully with correct tag nesting!")
